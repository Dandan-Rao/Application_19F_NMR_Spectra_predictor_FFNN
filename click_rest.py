...
@click.command()
@click.argument("smiles", type=str, default="C(=O)(C(C(C(C(C(C(C(F)(F)F)(F)F)(F)F)(F)F)(F)F)(F)F)(F)F)O")
def predict(smiles):
    ...
    if response.status_code == 200:
        try:
            result = response.json()  # This may throw an error if the response isn't valid JSON
            plot_data = result.get("plot_data", "No plot data available")
            table_data = result.get("table_data", "No table data available")
            structure_image_base64 = result.get("structure_image_base64", "No image available")
